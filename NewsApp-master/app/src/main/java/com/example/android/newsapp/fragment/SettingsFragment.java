...
public class SettingsFragment extends PreferenceFragmentCompat implements android.support.v7.preference.Preference.OnPreferenceChangeListener {
    ...
    private void createSettings(final Context context) {
        ...
        category.addPreference(createEditPreferenceSearch(context));
    ...
    private Preference createEditPreferenceSearch(Context context) {
        EditTextPreference search = new EditTextPreference(context);
        search.setKey("NEWSSEARCH");
        search.setDialogLayoutResource(R.layout.layout_string);
        search.setTitle(R.string.newssearch);
        search.setOnPreferenceChangeListener(this);
        search.setDefaultValue("");
        bindPreferenceSummaryToValue(context, search, "NEWSSEARCH", "");
        return search;
    }
    /**
